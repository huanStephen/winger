...
public class CodeAnalyzer implements Opcode {
	...
	private boolean processBranch(int opcode, CodeIterator ci, int index, int codeLength, int[] stack, int stackDepth,
			int[] jsrDepth) throws BadBytecode {
		int target;
		if ((153 > opcode || opcode > 166) && opcode != 198 && opcode != 199) {
			...
			case 201:
				...
				throw new BadBytecode(
						"sorry, cannot compute this data flow due to JSR: " + stackDepth + "," + jsrDepth[0]);
			case 169:
				if (jsrDepth[0] < 0) {
				...
				throw new BadBytecode(
						"sorry, cannot compute this data flow due to RET: " + stackDepth + "," + jsrDepth[0]);
			case 170:
			case 171:
