...
import org.eocencle.winger.javassist.bytecode.ByteArray;
...
import org.eocencle.winger.javassist.bytecode.StackMapTable.Writer;
import org.eocencle.winger.javassist.bytecode.stackmap.BasicBlock.Catch;
import org.eocencle.winger.javassist.bytecode.stackmap.BasicBlock.JsrBytecode;
import org.eocencle.winger.javassist.bytecode.stackmap.TypeData.AbsTypeVar;
import org.eocencle.winger.javassist.bytecode.stackmap.TypeData.BasicType;
import org.eocencle.winger.javassist.bytecode.stackmap.TypeData.ClassName;
...
public class MapMaker extends Tracer {
		...
		if (ca == null) {
			...
			if (blocks == null) {
				...
				try {
					...
					throw new BadBytecode(minfo, arg5);
				}
	...
	public MapMaker(ClassPool classes, MethodInfo minfo, CodeAttribute ca) {
		super(classes, minfo.getConstPool(), ca.getMaxStack(), ca.getMaxLocals(),
	...
	void make(TypedBlock[] blocks, byte[] code) throws BadBytecode {
		...
		try {
			...
			throw new BadBytecode("failed to resolve types", arg3);
		}
	}
	...
	private void traceException(byte[] code, Catch handler) throws BadBytecode {
	...
	private TypeData merge(TypeData src, TypeData target) throws BadBytecode {
		if (src == target) {
			return target;
		} else if (!(target instanceof ClassName) && !(target instanceof BasicType)) {
			if (target instanceof AbsTypeVar) {
				((AbsTypeVar) target).merge(src);
				return target;
			} else {
	...
	private ClassName toExceptionType(int exceptionType) {
		String type;
		if (exceptionType == 0) {
	...
	private boolean isTarget(TypedBlock cur, TypedBlock prev) {
		int in = cur.incoming;
		return in > 1 ? true : (in < 1 ? false : prev.stop);
	}

	private void toStackMapBody(Writer writer, TypedBlock bb, int diffL, int offsetDelta, TypedBlock prev) {
		...
		if (stackTop == 0) {
			...
			if (0 < diffL && diffL <= 3) {
				sdata = new int[diffL];
				stags = this.fillStackMap(bb.numLocals - prev.numLocals, prev.numLocals, sdata, bb.localsTypes);
				writer.appendFrame(offsetDelta, stags, sdata);
				return;
			}
		} else {
			TypeData sdata1;
			if (stackTop == 1 && diffL == 0) {
				sdata1 = bb.stackTypes[0];
				writer.sameLocals(offsetDelta, sdata1.getTypeTag(), sdata1.getTypeData(this.cpool));
				return;
			}

			if (stackTop == 2 && diffL == 0) {
				sdata1 = bb.stackTypes[0];
				if (sdata1.is2WordType()) {
					writer.sameLocals(offsetDelta, sdata1.getTypeTag(), sdata1.getTypeData(this.cpool));
					return;
				}
			}
		...
		sdata = new int[stackTop];
		stags = this.fillStackMap(stackTop, 0, sdata, bb.stackTypes);
	...
	private static int stackMapDiff(int oldTdLen, TypeData[] oldTd, int newTdLen, TypeData[] newTd) {
		...
		return stackMapEq(oldTd, newTd, len)
				? (diff > 0 ? diffSize(newTd, len, newTdLen) : -diffSize(oldTd, len, oldTdLen)) : -100;
	}
	...
	private void writeStackFrame(StackMap.Writer writer, ConstPool cp, int offset,
			TypedBlock tb) {
	...
	private void writeVerifyTypeInfo(StackMap.Writer writer, ConstPool cp,
			TypeData[] types, int num) {
		int numDWord = 0;
