...
public class BootstrapMethodsAttribute extends AttributeInfo {
	...
	public BootstrapMethodsAttribute(ConstPool cp, BootstrapMethodsAttribute.BootstrapMethod[] methods) {
		...
		for (int data = 0; data < methods.length; ++data) {
			size += 4 + methods[data].arguments.length * 2;
		}

		byte[] arg8 = new byte[size];
		ByteArray.write16bit(methods.length, arg8, 0);
		int pos = 2;

		for (int i = 0; i < methods.length; ++i) {
			ByteArray.write16bit(methods[i].methodRef, arg8, pos);
			ByteArray.write16bit(methods[i].arguments.length, arg8, pos + 2);
			int[] args = methods[i].arguments;
			pos += 4;

			for (int k = 0; k < args.length; ++k) {
				ByteArray.write16bit(args[k], arg8, pos);
				pos += 2;
			}
		}
