...
public class Bytecode extends ByteVector implements Cloneable, Opcode {
	...
	public Object clone() {
		try {
			Bytecode cnse = (Bytecode) super.clone();
			cnse.tryblocks = (ExceptionTable) this.tryblocks.clone();
			return cnse;
	...
	public int addLoad(int n, CtClass type) {
		if (type.isPrimitive()) {
			if (type != CtClass.booleanType && type != CtClass.charType && type != CtClass.byteType
					&& type != CtClass.shortType && type != CtClass.intType) {
				if (type == CtClass.longType) {
					this.addLload(n);
					return 2;
				}
