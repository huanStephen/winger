...
public class StackMapTable extends AttributeInfo {
	...
	public AttributeInfo copy(ConstPool newCp, Map classnames) throws StackMapTable.RuntimeCopyException {
		try {
			return new StackMapTable(newCp,
					(new StackMapTable.Copier(this.constPool, this.info, newCp, classnames)).doit());
	...
	public void removeNew(int where) throws CannotCompileException {
		try {
			byte[] e = (new StackMapTable.NewRemover(this.get(), where)).doit();
			this.set(e);
		} catch (BadBytecode arg2) {
			throw new CannotCompileException("bad stack map table", arg2);
		}
	}
	...
	static class NewRemover extends StackMapTable.SimpleCopy {
		...
		public void fullFrame(int pos, int offsetDelta, int[] localTags, int[] localData, int[] stackTags,
				int[] stackData) {
			int n = stackTags.length - 1;

			for (int i = 0; i < n; ++i) {
				if (stackTags[i] == 8 && stackData[i] == this.posOfNew && stackTags[i + 1] == 8
						&& stackData[i + 1] == this.posOfNew) {
					++n;
	...
	static class Printer extends StackMapTable.Walker {
		...
		public static void print(StackMapTable smt, PrintWriter writer) {
			try {
				...
				writer.println(arg2.getMessage());
			}
	...
	static class SimpleCopy extends StackMapTable.Walker {
		...
		public void fullFrame(int pos, int offsetDelta, int[] localTags, int[] localData, int[] stackTags,
				int[] stackData) {
			this.writer.fullFrame(offsetDelta, localTags, this.copyData(localTags, localData), stackTags,
					this.copyData(stackTags, stackData));
		}
	...
	public static class Walker {
		...
		public void fullFrame(int pos, int offsetDelta, int[] localTags, int[] localData, int[] stackTags,
				int[] stackData) throws BadBytecode {
		}
