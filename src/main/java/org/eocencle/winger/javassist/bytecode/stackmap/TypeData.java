...
public abstract class TypeData {
	...
	public abstract int getTypeData(ConstPool arg0);
	...
	public abstract boolean eq(TypeData arg0);

	public abstract String getName();

	public abstract void setType(String arg0, ClassPool arg1) throws BadBytecode;
	...
	public static CtClass commonSuperClassEx(CtClass one, CtClass two) throws NotFoundException {
		if (one == two) {
			...
			return element == ele1 ? one
					: (element == ele2 ? two
							: one.getClassPool().get(element == null ? "java.lang.Object" : element.getName() + "[]"));
		} else {
			return !one.isPrimitive() && !two.isPrimitive() ? (!one.isArray() && !two.isArray()
					? commonSuperClass(one, two) : one.getClassPool().get("java.lang.Object")) : null;
		}
	}
	...
	public static class UninitData extends TypeData.ClassName {
		...
		public TypeData join() {
			return (TypeData) (this.initialized ? new TypeData.TypeVar(new TypeData.ClassName(this.getName()))
					: new TypeData.UninitTypeVar(this.copy()));
		}
	...
	public static class ArrayElement extends TypeData.AbsTypeVar {
		...
		public void merge(TypeData t) {
			try {
				...
				throw new RuntimeException("fatal: " + arg2);
			}
		}
	...
	public static class TypeVar extends TypeData.AbsTypeVar {
		...
		public int dfs(ArrayList preOrder, int index, ClassPool cp) throws NotFoundException {
			if (this.visited > 0) {
				...
				for (int scc = 0; scc < n; ++scc) {
					cv = ((TypeData) this.lowers.get(scc)).toTypeVar();
					if (cv != null) {
						if (cv.visited == 0) {
		...
		private void fixTypes(ArrayList scc, ClassPool cp) throws NotFoundException {
			...
			int typeName;
			for (typeName = 0; typeName < size; ++typeName) {
				ArrayList i = ((TypeData.TypeVar) scc.get(typeName)).lowers;
				int cv = i.size();

				for (int j = 0; j < cv; ++j) {
					TypeData d = (TypeData) i.get(j);
			...
			if (isBasicType) {
				for (typeName = 0; typeName < size; ++typeName) {
					TypeData.TypeVar arg13 = (TypeData.TypeVar) scc.get(typeName);
					arg13.lowers.clear();
					arg13.lowers.add(kind);
					this.is2WordType = ((TypeData) kind).is2WordType();
				}
			} else {
				String arg12 = this.fixTypes2(scc, lowersSet, cp);

				for (int arg14 = 0; arg14 < size; ++arg14) {
					TypeData.TypeVar arg15 = (TypeData.TypeVar) scc.get(arg14);
					arg15.fixedType = arg12;
				}
			}
		...
		private String fixTypes2(ArrayList scc, HashSet lowersSet, ClassPool cp) throws NotFoundException {
			...
			if (lowersSet.size() == 0) {
				...
				CtClass cc;
				for (cc = cp.get((String) it.next()); it
						.hasNext(); cc = commonSuperClassEx(cc, cp.get((String) it.next()))) {
					;
				}
	...
	public abstract static class AbsTypeVar extends TypeData {
		public abstract void merge(TypeData arg0);
