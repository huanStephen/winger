...
public class CodeAttribute extends AttributeInfo implements Opcode {
	...
	private static CodeAttribute.LdcEntry copyCode(byte[] code, int beginPos, int endPos, ConstPool srcCp,
			byte[] newcode, ConstPool destCp, Map classnameMap) throws BadBytecode {
	...
	private static void copyConstPoolInfo(int i, byte[] code, ConstPool srcCp, byte[] newcode, ConstPool destCp,
			Map classnameMap) {
	...
	private static void shiftIndex0(CodeIterator ci, int index, int opcode, int lessThan, int delta, int opcode_i_0,
			int opcode_i) throws BadBytecode {
	...
	static class LdcEntry {
		...
		static byte[] doit(byte[] code, CodeAttribute.LdcEntry ldc, ExceptionTable etable, CodeAttribute ca)
				throws BadBytecode {
			if (ldc != null) {
