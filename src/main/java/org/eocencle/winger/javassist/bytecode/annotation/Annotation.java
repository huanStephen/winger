...
public class Annotation {
	...
	public static MemberValue createMemberValue(ConstPool cp, CtClass type) throws NotFoundException {
		if (type == CtClass.booleanType) {
			...
			CtClass emv2 = type.getComponentType();
			MemberValue member = createMemberValue(cp, emv2);
			return new ArrayMemberValue(member, cp);
		} else if (type.isInterface()) {
			Annotation emv1 = new Annotation(cp, type);
			return new AnnotationMemberValue(emv1, cp);
		} else {
	...
	public boolean equals(Object obj) {
		if (obj == this) {
			...
			if (!this.getTypeName().equals(other.getTypeName())) {
				...
				return this.members == otherMembers ? true
						: (this.members == null ? otherMembers == null
								: (otherMembers == null ? false : this.members.equals(otherMembers)));
			}
		} else {
			return false;
		}
