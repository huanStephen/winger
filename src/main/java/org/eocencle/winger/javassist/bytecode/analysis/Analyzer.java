...
public class Analyzer implements Opcode {
	...
	private void analyzeNextEntry(MethodInfo method, CodeIterator iter, IntQueue queue, Executor executor)
			throws BadBytecode {
		int pos = queue.take();
		iter.move(pos);
		iter.next();
		...
		try {
			...
			throw new BadBytecode(arg9.getMessage() + "[pos = " + pos + "]", arg9);
		}
	...
	private Analyzer.ExceptionInfo[] buildExceptionInfo(MethodInfo method) {
		...
		for (int i = 0; i < table.size(); ++i) {
			...
			try {
				...
				throw new IllegalStateException(arg9.getMessage());
			}
	...
	private void mergeLookupSwitch(IntQueue queue, int pos, CodeIterator iter, Frame frame) throws BadBytecode {
		...
		int arg9999 = npairs * 8;
		index += 4;
		int end = arg9999 + index;
	...
	private void mergeTableSwitch(IntQueue queue, int pos, CodeIterator iter, Frame frame) throws BadBytecode {
		...
		int arg9999 = (high - low + 1) * 4;
		index += 4;

		for (int end = arg9999 + index; index < end; index += 4) {
			int target = iter.s32bitAt(index) + pos;
			this.merge(queue, frame, target);
		}
