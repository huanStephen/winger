...
public class Executor implements Opcode {
	...
	public void execute(MethodInfo method, int pos, CodeIterator iter, Frame frame, Subroutine subroutine)
		...
		case 91:
			...
			arg13 = frame.getTopIndex();
			int arg14 = arg13 - (opcode - 90) - 1;
			frame.push(array);

			while (arg13 > arg14) {
				frame.setStack(arg13, frame.getStack(arg13 - 1));
				--arg13;
			}

			frame.setStack(arg14, array);
			break;
		case 92:
		...
		case 93:
		case 94:
			arg12 = frame.getTopIndex();
			arg13 = arg12 - (opcode - 93) - 1;
			Type type1 = frame.getStack(frame.getTopIndex() - 1);
			Type type21 = frame.peek();
			frame.push(type1);
			frame.push(type21);

			while (arg12 > arg13) {
				frame.setStack(arg12, frame.getStack(arg12 - 2));
				--arg12;
			}

			frame.setStack(arg13, type21);
			frame.setStack(arg13 - 1, type1);
			break;
		case 95:
			array = frame.pop();
			Type type2 = frame.pop();
			if (array.getSize() != 2 && type2.getSize() != 2) {
				frame.push(array);
				frame.push(type2);
				break;
			}
		...
		case 131:
			...
			break;
		case 132:
			arg12 = iter.byteAt(pos + 1);
			this.verifyAssignable(Type.INTEGER, frame.getLocal(arg12));
			this.access(arg12, Type.INTEGER, subroutine);
			break;
		case 133:
		...
		case 176:
			try {
				CtClass arg11 = Descriptor.getReturnType(method.getDescriptor(), this.classPool);
				this.verifyAssignable(Type.get(arg11), this.simplePop(frame));
				break;
	...
	private Type zeroExtend(Type type) {
		return type != Type.SHORT && type != Type.BYTE && type != Type.CHAR && type != Type.BOOLEAN ? type
				: Type.INTEGER;
	}
	...
	private Type[] paramTypesFromDesc(String desc) throws BadBytecode {
		...
		try {
			...
			throw new BadBytecode(
					"Could not find class in descriptor [pos = " + this.lastPos + "]: " + arg4.getMessage());
		}
	...
	private Type returnTypeFromDesc(String desc) throws BadBytecode {
		...
		try {
			...
			throw new BadBytecode(
					"Could not find class in descriptor [pos = " + this.lastPos + "]: " + arg3.getMessage());
		}
