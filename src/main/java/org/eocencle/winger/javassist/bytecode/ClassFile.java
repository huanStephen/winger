...
public final class ClassFile {
	...
	public void compact() {
		...
		int i;
		for (i = 0; i < n; ++i) {
			MethodInfo finfo = (MethodInfo) list.get(i);
			finfo.compact(cp);
		}
		...
		for (i = 0; i < n; ++i) {
			FieldInfo arg5 = (FieldInfo) list.get(i);
			arg5.compact(cp);
		}
	...
	public void prune() {
		...
		for (i = 0; i < n; ++i) {
			FieldInfo arg9 = (FieldInfo) list.get(i);
			arg9.prune(cp);
		}
	...
	public void setSuperclass(String superclass) throws CannotCompileException {
		...
		try {
			this.superClass = this.constPool.addClassInfo(superclass);
			ArrayList e = this.methods;
			int n = e.size();

			for (int i = 0; i < n; ++i) {
				MethodInfo minfo = (MethodInfo) e.get(i);
	...
	public final void renameClass(String oldname, String newname) {
		if (!oldname.equals(newname)) {
			...
			for (i = 0; i < n; ++i) {
				FieldInfo arg7 = (FieldInfo) list.get(i);
				desc = arg7.getDescriptor();
				arg7.setDescriptor(Descriptor.rename(desc, oldname, newname));
				AttributeInfo.renameClass(arg7.getAttributes(), oldname, newname);
			}
	...
	public final void renameClass(Map classnames) {
		...
		for (i = 0; i < n; ++i) {
			FieldInfo arg7 = (FieldInfo) list.get(i);
			desc = arg7.getDescriptor();
			arg7.setDescriptor(Descriptor.rename(desc, classnames));
			AttributeInfo.renameClass(arg7.getAttributes(), classnames);
		}
	...
	public final void getRefClasses(Map classnames) {
		...
		for (i = 0; i < n; ++i) {
			FieldInfo arg6 = (FieldInfo) list.get(i);
			desc = arg6.getDescriptor();
			Descriptor.rename(desc, classnames);
			AttributeInfo.getRefClasses(arg6.getAttributes(), classnames);
		}
	...
	public void write(DataOutputStream out) throws IOException {
		...
		for (i = 0; i < n; ++i) {
			MethodInfo arg5 = (MethodInfo) list.get(i);
			arg5.write(out);
		}
