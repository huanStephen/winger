...
public class ClassPoolTail {
	...
	private static ClassPath makePathObject(String pathname) throws NotFoundException {
		...
		if (!lower.endsWith(".jar") && !lower.endsWith(".zip")) {
			...
			if (len <= 2 || pathname.charAt(len - 1) != 42
					|| pathname.charAt(len - 2) != 47 && pathname.charAt(len - 2) != File.separatorChar) {
	...
	public static byte[] readStream(InputStream fin) throws IOException {
		...
		for (int i = 0; i < 8; ++i) {
			...
			do {
				int arg8 = fin.read(bufs[i], size, bufsize - size);
				if (arg8 < 0) {
	...
	public static void copyStream(InputStream fin, OutputStream fout) throws IOException {
		...
		for (int i = 0; i < 64; ++i) {
			...
			do {
				int arg6 = fin.read(buf, size, bufsize - size);
				if (arg6 < 0) {
