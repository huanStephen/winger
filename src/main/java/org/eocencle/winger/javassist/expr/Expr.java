...
import org.eocencle.winger.javassist.expr.ExprEditor.LoopContext;
...
public abstract class Expr implements Opcode {
	...
	public CtBehavior where() {
		...
		for (int init = cb.length - 1; init >= 0; --init) {
			if (cb[init].getMethodInfo2() == mi) {
				return cb[init];
			}
		}

		CtConstructor arg4 = this.thisClass.getClassInitializer();
		if (arg4 != null && arg4.getMethodInfo2() == mi) {
			return arg4;
		} else {
			for (int i = cb.length - 1; i >= 0; --i) {
				if (this.thisMethod.getName().equals(cb[i].getMethodInfo2().getName())
						&& this.thisMethod.getDescriptor().equals(cb[i].getMethodInfo2().getDescriptor())) {
					return cb[i];
				}
			}
	...
	public CtClass[] mayThrow() {
		...
		int i;
		try {
			CodeAttribute ea = this.thisMethod.getCodeAttribute();
			ExceptionTable exceptions = ea.getExceptionTable();
			n = this.currentPos;
			i = exceptions.size();

			for (int i1 = 0; i1 < i; ++i1) {
				if (exceptions.startPc(i1) <= n && n < exceptions.endPc(i1)) {
					int t = exceptions.catchType(i1);
					if (t > 0) {
						try {
		...
		} catch (NullPointerException arg12) {
			;
		}

		ExceptionsAttribute arg13 = this.thisMethod.getExceptionsAttribute();
		if (arg13 != null) {
			String[] arg14 = arg13.getExceptions();
			if (arg14 != null) {
				n = arg14.length;

				for (i = 0; i < n; ++i) {
					try {
						addClass(list, pool.get(arg14[i]));
	...
	public abstract void replace(String arg0) throws CannotCompileException;
	...
	protected void runEditor(ExprEditor ed, CodeIterator oldIterator) throws CannotCompileException {
		...
		LoopContext context = new LoopContext(newLocals);
