...
public class TypeChecker extends Visitor implements Opcode, TokenId {
	...
	public void atBinExpr(BinExpr expr) throws CompileError {
		...
		if (k >= 0) {
			if (token == 43) {
				Expr left = this.atPlusExpr(expr);
				if (left != null) {
					CallExpr left1 = CallExpr.makeCall(Expr.make(46, left, new Member("toString")), (ASTree) null);
					expr.setOprand1(left1);
					expr.setOprand2((ASTree) null);
					this.className = "java/lang/String";
				}
			} else {
				ASTree left2 = expr.oprand1();
				ASTree right = expr.oprand2();
				left2.accept(this);
				int type1 = this.exprType;
				right.accept(this);
				if (!this.isConstant(expr, token, left2, right)) {
	...
	private Expr atPlusExpr(BinExpr expr) throws CompileError {
		...
		if (right == null) {
			...
			if (isPlusExpr(left)) {
				Expr type1 = this.atPlusExpr((BinExpr) left);
				if (type1 != null) {
					right.accept(this);
					this.exprType = 307;
					this.arrayDim = 0;
					this.className = "java/lang/StringBuffer";
					return makeAppendCall(type1, right);
				}
			} else {
				left.accept(this);
			}

			int type11 = this.exprType;
			int dim1 = this.arrayDim;
			String cname = this.className;
			right.accept(this);
			if (this.isConstant(expr, 43, left, right)) {
				return null;
			} else if ((type11 != 307 || dim1 != 0 || !"java/lang/String".equals(cname))
					&& (this.exprType != 307 || this.arrayDim != 0 || !"java/lang/String".equals(this.className))) {
				this.computeBinExprType(expr, 43, type11);
				return null;
			} else {
				ASTList sbufClass = ASTList.make(new Symbol("java"), new Symbol("lang"), new Symbol("StringBuffer"));
				NewExpr e = new NewExpr(sbufClass, (ASTList) null);
				this.exprType = 307;
				this.arrayDim = 0;
				this.className = "java/lang/StringBuffer";
	...
	static ASTree stripPlusExpr(ASTree expr) {
		if (expr instanceof BinExpr) {
			BinExpr cexpr = (BinExpr) expr;
			if (cexpr.getOperator() == 43 && cexpr.oprand2() == null) {
				return cexpr.getLeft();
			}
		} else if (expr instanceof Expr) {
			Expr cexpr2 = (Expr) expr;
			int op = cexpr2.getOperator();
			if (op == 35) {
				ASTree cexpr1 = getConstantFieldValue((Member) cexpr2.oprand2());
				if (cexpr1 != null) {
					return cexpr1;
				}
			} else if (op == 43 && cexpr2.getRight() == null) {
				return cexpr2.getLeft();
			}
		} else if (expr instanceof Member) {
			ASTree cexpr3 = getConstantFieldValue((Member) expr);
			if (cexpr3 != null) {
				return cexpr3;
			}
		}
			...
			if (value == null) {
				...
				if (!(value instanceof Double) && !(value instanceof Float)) {
					if (value instanceof Number) {
						...
						return value instanceof Boolean ? new Keyword(((Boolean) value).booleanValue() ? 410 : 411)
								: null;
					}
				} else {
	...
	public void atExpr(Expr expr) throws CompileError {
		...
		if (token == 46) {
			...
			if (token == 33) {
				...
				oprand.accept(this);
				if (!this.isConstant(expr, token, oprand) && (token == 45 || token == 126)
	...
	private boolean isConstant(Expr expr, int op, ASTree oprand) {
		...
		if (oprand instanceof IntConst) {
			...
			DoubleConst c1 = (DoubleConst) oprand;
			if (op != 45) {
				return false;
			}
	...
	public void atCallExpr(CallExpr expr) throws CompileError {
		...
		if (method instanceof Member) {
			...
			Expr minfo = (Expr) method;
			mname = ((Symbol) minfo.oprand2()).get();
			int op = minfo.getOperator();
			if (op == 35) {
				targetClass = this.resolver.lookupClass(((Symbol) minfo.oprand1()).get(), false);
			} else if (op != 46) {
				badMethod();
			} else {
				ASTree target = minfo.oprand1();
				...
				if (classFollowedByDotSuper != null) {
					...
					try {
						target.accept(this);
					} catch (NoFieldException arg10) {
						if (arg10.getExpr() != target) {
							throw arg10;
						}

						this.exprType = 307;
						this.arrayDim = 0;
						this.className = arg10.getField();
						minfo.setOperator(35);
						minfo.setOprand1(new Symbol(MemberResolver.jvmToJavaName(this.className)));
					}
		...
		Method minfo1 = this.atMethodCallCore(targetClass, mname, args);
		expr.setMethod(minfo1);
	}
	...
	public Method atMethodCallCore(CtClass targetClass, String mname, ASTList args) throws CompileError {
		...
		Method found = this.resolver.lookupMethod(targetClass, this.thisClass, this.thisMethod, mname, types, dims,
				cnames);
		String desc;
		if (found == null) {
			desc = targetClass.getName();
			String signature = argTypesToString(types, dims, cnames);
			String msg;
			if (mname.equals("<init>")) {
				msg = "cannot find constructor " + desc + signature;
			} else {
				msg = mname + signature + " not found in " + desc;
			}

			throw new CompileError(msg);
		} else {
			desc = found.info.getDescriptor();
	...
	protected CtField fieldAccess(ASTree expr) throws CompileError {
		if (expr instanceof Member) {
			Member e1 = (Member) expr;
			String op1 = e1.get();

			try {
				CtField err2 = this.thisClass.getField(op1);
				if (Modifier.isStatic(err2.getModifiers())) {
					e1.setField(err2);
				}

				return err2;
			} catch (NotFoundException arg5) {
				throw new NoFieldException(op1, expr);
			}
		} else {
			if (expr instanceof Expr) {
				...
				if (op == 35) {
					Member err1 = (Member) e.oprand2();
					CtField oprnd11 = this.resolver.lookupField(((Symbol) e.oprand1()).get(), err1);
					err1.setField(oprnd11);
					return oprnd11;
				}
				...
				if (op == 46) {
					...
					} catch (NoFieldException arg7) {
						if (arg7.getExpr() != e.oprand1()) {
							throw arg7;
						}

						return this.fieldAccess2(e, arg7.getField());
					}
	...
	private void atPlusPlus(int token, ASTree oprand, Expr expr) throws CompileError {
		...
		if (oprand instanceof Variable) {
			...
			this.exprType = e.getType();
			this.arrayDim = e.getArrayDim();
		} else {
			if (oprand instanceof Expr) {
				Expr e1 = (Expr) oprand;
				if (e1.getOperator() == 65) {
					this.atArrayRead(e1.oprand1(), e1.oprand2());
					int t = this.exprType;
