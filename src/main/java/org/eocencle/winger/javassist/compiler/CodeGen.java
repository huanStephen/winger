...
import org.eocencle.winger.javassist.compiler.ast.AssignExpr;
import org.eocencle.winger.javassist.compiler.ast.BinExpr;
import org.eocencle.winger.javassist.compiler.ast.CallExpr;
import org.eocencle.winger.javassist.compiler.ast.CastExpr;
import org.eocencle.winger.javassist.compiler.ast.CondExpr;
import org.eocencle.winger.javassist.compiler.ast.Declarator;
import org.eocencle.winger.javassist.compiler.ast.DoubleConst;
import org.eocencle.winger.javassist.compiler.ast.Expr;
import org.eocencle.winger.javassist.compiler.ast.FieldDecl;
import org.eocencle.winger.javassist.compiler.ast.InstanceOfExpr;
import org.eocencle.winger.javassist.compiler.ast.IntConst;
...
import org.eocencle.winger.javassist.compiler.ast.Variable;
import org.eocencle.winger.javassist.compiler.ast.Visitor;
import org.eocencle.winger.javassist.compiler.ast.StringL;
...
public abstract class CodeGen extends Visitor implements Opcode, TokenId {
	...
	static final int[] binOp = new int[] { 43, 99, 98, 97, 96, 45, 103, 102, 101, 100, 42, 107, 106, 105, 104, 47, 111,
			110, 109, 108, 37, 115, 114, 113, 112, 124, 0, 0, 129, 128, 94, 0, 0, 131, 130, 38, 0, 0, 127, 126, 364, 0,
			0, 121, 120, 366, 0, 0, 123, 122, 370, 0, 0, 125, 124 };
	private static final int[] ifOp = new int[] { 358, 159, 160, 350, 160, 159, 357, 164, 163, 359, 162, 161, 60, 161,
			162, 62, 163, 164 };
	private static final int[] ifOp2 = new int[] { 358, 153, 154, 350, 154, 153, 357, 158, 157, 359, 156, 155, 60, 155,
			156, 62, 157, 158 };
	...
	private static final int[] castOp = new int[] { 0, 144, 143, 142, 141, 0, 140, 139, 138, 137, 0, 136, 135, 134, 133,
			0 };
	...
	protected abstract String resolveClassName(ASTList arg0) throws CompileError;

	protected abstract String resolveClassName(String arg0) throws CompileError;
	...
	public void atMethodDecl(MethodDecl method) throws CompileError {
		...
		while (mods != null) {
			Keyword params = (Keyword) mods.head();
			mods = mods.tail();
			if (params.get() == 335) {
				this.setMaxLocals(0);
		...
		for (ASTList params1 = method.getParams(); params1 != null; params1 = params1.tail()) {
			this.atDeclarator((Declarator) params1.head());
		}
	...
	public void atStmnt(Stmnt st) throws CompileError {
		if (st != null) {
			...
			if (op == 69) {
				ASTree list = st.getLeft();
				this.doTypeCheck(list);
				if (list instanceof AssignExpr) {
					this.atAssignExpr((AssignExpr) list, false);
				} else if (isPlusPlusExpr(list)) {
					Expr h = (Expr) list;
					this.atPlusPlus(h.getOperator(), h.oprand1(), h, false);
				} else {
					list.accept(this);
				...
				while (list1 != null) {
					ASTree h1 = ((ASTList) list1).head();
					list1 = ((ASTList) list1).tail();
					if (h1 != null) {
						h1.accept(this);
					}
				}
	...
	private void atSwitchStmnt(Stmnt st) throws CompileError {
		...
		for (Object opcodePc2 = body; opcodePc2 != null; opcodePc2 = ((ASTList) opcodePc2).tail()) {
			if (((Stmnt) ((ASTList) opcodePc2).head()).getOperator() == 304) {
				++npairs;
			}
		}

		int arg13 = this.bytecode.currentPc();
		...
		for (Object pc = body; pc != null; pc = ((ASTList) pc).tail()) {
			Stmnt endPc = (Stmnt) ((ASTList) pc).head();
			int op = endPc.getOperator();
			if (op == 310) {
				defaultPc = this.bytecode.currentPc();
			} else if (op != 304) {
				fatal();
			} else {
				pairs[ipairs++] = ((long) this.computeLabel(endPc.head()) << 32)
						+ ((long) (this.bytecode.currentPc() - opcodePc) & -1L);
			}

			this.hasReturned = false;
			((Stmnt) endPc.tail()).accept(this);
		}
		...
		int arg15;
		for (arg15 = 0; arg15 < npairs; ++arg15) {
			this.bytecode.write32bit(arg14, (int) (pairs[arg15] >>> 32));
			this.bytecode.write32bit(arg14 + 4, (int) pairs[arg15]);
			arg14 += 8;
		}
		...
		arg15 = this.bytecode.currentPc();
		if (defaultPc < 0) {
			defaultPc = arg15;
		}

		this.bytecode.write32bit(arg13, defaultPc - opcodePc);
		this.patchGoto(this.breakList, arg15);
		this.breakList = prevBreakList;
	}
	...
	protected final void atReturnStmnt2(ASTree result) throws CompileError {
		...
		for (CodeGen.ReturnHook har1 = this.returnHooks; har1 != null; har1 = har1.next) {
			if (har1.doit(this.bytecode, op)) {
				this.hasReturned = true;
				return;
			}
		}
	...
	public abstract void atNewExpr(NewExpr arg0) throws CompileError;

	public abstract void atArrayInit(ArrayInit arg0) throws CompileError;
	...
	protected abstract void atArrayVariableAssign(ArrayInit arg0, int arg1, int arg2, String arg3) throws CompileError;
	...
	protected abstract void atFieldAssign(Expr arg0, int arg1, ASTree arg2, ASTree arg3, boolean arg4)
			throws CompileError;
	...
	protected void atAssignCore(Expr expr, int op, ASTree right, int type, int dim, String cname) throws CompileError {
		if (op == 354 && dim == 0 && type == 307) {
			...
			right.accept(this);
			if (this.invalidDim(this.exprType, this.arrayDim, this.className, type, dim, cname, false)
					|| op != 61 && dim > 0) {
				badAssign(expr);
			}
	...
	private boolean invalidDim(int srcType, int srcDim, String srcClass, int destType, int destDim, String destClass,
			boolean isCast) {
		return srcDim != destDim
				? (srcType == 412 ? false
						: (destDim == 0 && destType == 307 && "java/lang/Object".equals(destClass) ? false
								: !isCast || srcDim != 0 || srcType != 307 || !"java/lang/Object".equals(srcClass)))
				: false;
	}
	...
	static int getCompOperator(ASTree expr) throws CompileError {
		if (expr instanceof Expr) {
			...
			return token == 33 ? 33
					: (bexpr instanceof BinExpr && token != 368 && token != 369 && token != 38 && token != 124 ? 358
							: token);
		} else {
			return 32;
		}
	...
	private static int typePrecedence(int type) {
		return type == 312 ? 0
				: (type == 317 ? 1 : (type == 326 ? 2 : (isRefType(type) ? -1 : (type == 344 ? -1 : 3))));
	}
	...
	private String checkCastExpr(CastExpr expr, String name) throws CompileError {
		...
		oprand.accept(this);
		int srcType = this.exprType;
		if (!this.invalidDim(srcType, this.arrayDim, this.className, type, dim, name, true) && srcType != 344
				&& type != 344) {
			if (type == 307) {
	...
	public void atExpr(Expr expr) throws CompileError {
		...
		if (token == 46) {
			String type = ((Symbol) expr.oprand2()).get();
			...
			if (token == 33) {
				...
				expr.oprand1().accept(this);
				int type1 = typePrecedence(this.exprType);
				if (this.arrayDim > 0) {
					badType(expr);
				}
	...
	public abstract void atCallExpr(CallExpr arg0) throws CompileError;

	protected abstract void atFieldRead(ASTree arg0) throws CompileError;
	...
	protected void atClassObject2(String cname) throws CompileError {
		...
		this.bytecode.addInvokestatic("org.eocencle.winger.javassist.runtime.DotClass", "fail",
	...
	private void atPlusPlus(int token, ASTree oprand, Expr expr, boolean doDup) throws CompileError {
		...
		if (oprand instanceof Variable) {
			Declarator e = ((Variable) oprand).getDeclarator();
			int t = this.exprType = e.getType();
			this.arrayDim = e.getArrayDim();
			int var = this.getLocalVar(e);
			if (this.arrayDim > 0) {
				badType(expr);
			}
			...
			if (oprand instanceof Expr) {
				Expr e1 = (Expr) oprand;
				if (e1.getOperator() == 65) {
					this.atArrayPlusPlus(token, isPost, e1, doDup);
					return;
				}
			}
	...
	protected abstract void atFieldPlusPlus(int arg0, boolean arg1, ASTree arg2, Expr arg3, boolean arg4)
			throws CompileError;

	public abstract void atMember(Member arg0) throws CompileError;
	...
	protected abstract static class ReturnHook {
		...
		protected abstract boolean doit(Bytecode arg0, int arg1);
