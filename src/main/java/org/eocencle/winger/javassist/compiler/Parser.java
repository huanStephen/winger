...
public final class Parser implements TokenId {
	private Lex lex;
	private static final int[] binaryOpPrecedence = new int[] { 0, 0, 0, 0, 1, 6, 0, 0, 0, 1, 2, 0, 2, 0, 1, 0, 0, 0, 0,
	...
	public ASTList parseMember1(SymbolTable tbl) throws CompileError {
		...
		if (this.lex.get() != 400) {
			...
			return (ASTList) (!isConstructor && this.lex.lookAhead() != 40 ? this.parseField(tbl, mods, d)
					: this.parseMethod1(tbl, isConstructor, mods, d));
		}
	}
	...
	private MethodDecl parseMethod1(SymbolTable tbl, boolean isConstructor, ASTList mods, Declarator d)
		...
		if (this.lex.get() != 40) {
			...
			if (this.lex.lookAhead() != 41) {
				label39: while (true) {
					while (true) {
						...
						int throwsList = this.lex.lookAhead();
						if (throwsList == 44) {
							this.lex.get();
						} else if (throwsList == 41) {
							break label39;
						}
					}
				}
			...
			if (isConstructor && d.getArrayDim() > 0) {
				...
				ASTList throwsList1 = null;
				if (this.lex.lookAhead() == 341) {
					this.lex.get();

					while (true) {
						throwsList1 = ASTList.append(throwsList1, this.parseClassType(tbl));
				...
				return new MethodDecl(mods, new ASTList(d, ASTList.make(parms, throwsList1, (ASTree) null)));
			}
		}
	}
	...
	private ASTList parseMemberMods() {
		...
		while (true) {
			...
			if (t != 300 && t != 315 && t != 332 && t != 331 && t != 330 && t != 338 && t != 335 && t != 345 && t != 342
					&& t != 347) {
				return list;
			}
	...
	private Declarator parseFormalType(SymbolTable tbl) throws CompileError {
		...
		if (!isBuiltinType(t) && t != 344) {
			ASTList name1 = this.parseClassType(tbl);
			int dim = this.parseArrayDimension();
			return new Declarator(name1, dim);
		} else {
			this.lex.get();
			int name = this.parseArrayDimension();
			return new Declarator(t, name);
		}
	}
	...
	public Stmnt parseStatement(SymbolTable tbl) throws CompileError {
		...
		if (t == 123) {
			...
			return t == 320 ? this.parseIf(tbl)
					: (t == 346 ? this.parseWhile(tbl)
							: (t == 311 ? this.parseDo(tbl)
									: (t == 318 ? this.parseFor(tbl)
											: (t == 343 ? this.parseTry(tbl)
													: (t == 337 ? this.parseSwitch(tbl)
															: (t == 338 ? this.parseSynchronized(tbl)
																	: (t == 333 ? this.parseReturn(tbl)
																			: (t == 340 ? this.parseThrow(tbl)
																					: (t == 302 ? this.parseBreak(tbl)
																							: (t == 309
																									? this.parseContinue(
																											tbl)
																									: this.parseDeclarationOrExpression(
																											tbl,
																											false)))))))))));
		}
	}
	...
	private Stmnt parseTry(SymbolTable tbl) throws CompileError {
		...
		for (catchList = null; this.lex.lookAhead() == 305; catchList = ASTList.append(catchList, new Pair(d, b))) {
			...
			SymbolTable finallyBlock = new SymbolTable(tbl);
			d = this.parseFormalParam(finallyBlock);
			...
			b = this.parseBlock(finallyBlock);
		}

		Stmnt finallyBlock1 = null;
		if (this.lex.lookAhead() == 316) {
			this.lex.get();
			finallyBlock1 = this.parseBlock(tbl);
		}

		return Stmnt.make(343, block, catchList, finallyBlock1);
	}
	...
	private Stmnt parseDeclarationOrExpression(SymbolTable tbl, boolean exprList) throws CompileError {
		...
		if (isBuiltinType(t)) {
			t = this.lex.get();
			expr = this.parseArrayDimension();
			return this.parseDeclarators(tbl, new Declarator(t, expr));
		} else {
			if (t == 400) {
				expr = this.nextIsClassType(0);
				if (expr >= 0 && this.lex.lookAhead(expr) == 400) {
			...
			Stmnt expr1;
			if (exprList) {
				expr1 = this.parseExprList(tbl);
			} else {
				expr1 = new Stmnt(69, this.parseExpression(tbl));
			}
	...
	private static boolean isAssignOp(int t) {
		return t == 61 || t == 351 || t == 352 || t == 353 || t == 354 || t == 355 || t == 356 || t == 360 || t == 361
				|| t == 365 || t == 367 || t == 371;
	}
	...
	private ASTree parseInstanceOf(SymbolTable tbl, ASTree expr) throws CompileError {
		...
		if (isBuiltinType(t)) {
			this.lex.get();
			int name1 = this.parseArrayDimension();
			return new InstanceOfExpr(t, name1, expr);
		} else {
			ASTList name = this.parseClassType(tbl);
			int dim = this.parseArrayDimension();
	...
	private int getOpPrecedence(int c) {
		return 33 <= c
				&& c <= 63
						? binaryOpPrecedence[c - 33]
						: (c == 94 ? 7
								: (c == 124 ? 8
										: (c == 369 ? 9
												: (c == 368 ? 10
														: (c != 358 && c != 350 ? (c != 357 && c != 359 && c != 323
																? (c != 364 && c != 366 && c != 370 ? 0 : 3) : 4)
																: 5)))));
	}
	...
	private ASTree parseCast(SymbolTable tbl) throws CompileError {
		...
		if (isBuiltinType(t) && this.nextIsBuiltinCast()) {
			...
			if (this.lex.get() != 41) {
				...
				return new CastExpr(t, name1, this.parseUnaryExpr(tbl));
			}
	...
	private boolean nextIsClassCast() {
		...
		if (i < 0) {
			...
			if (t != 41) {
				...
				return t == 40 || t == 412 || t == 406 || t == 400 || t == 339 || t == 336 || t == 328 || t == 410
						|| t == 411 || t == 403 || t == 402 || t == 401 || t == 405 || t == 404;
			}
		}
	}
