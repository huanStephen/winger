...
public class TransformCall extends Transformer {
	...
	private boolean matchClass(String name, ClassPool pool) {
		if (this.classname.equals(name)) {
			...
			try {
				CtClass e = pool.get(name);
				CtClass declClazz = pool.get(this.classname);
				if (e.subtypeOf(declClazz)) {
					try {
						CtMethod e1 = e.getMethod(this.methodname, this.methodDescriptor);
						return e1.getDeclaringClass().getName().equals(this.classname);
