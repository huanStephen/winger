...
import org.eocencle.winger.javassist.CodeConverter.ArrayAccessReplacementMethodNames;
...
import org.eocencle.winger.javassist.bytecode.analysis.Analyzer;
import org.eocencle.winger.javassist.bytecode.analysis.Frame;
...
public class TransformAccessArrayField extends Transformer {
			...
			try {
				...
				if (c != 50 && c != 51 && c != 52 && c != 49 && c != 48 && c != 46 && c != 47 && c != 53) {
					if (c == 83 || c == 84 || c == 85 || c == 82 || c == 81 || c == 79 || c == 80 || c == 86) {
						this.replace(cp, iterator, e, c, this.getStoreReplacementSignature(c));
					}
				} else {
					this.replace(cp, iterator, e, c, this.getLoadReplacementSignature(c));
				}
	...
	private int replace(ConstPool cp, CodeIterator iterator, int pos, int opcode, String signature) throws BadBytecode {
		...
		if (methodName != null) {
			...
			Gap gap = iterator.insertGapAt(pos, castType != null ? 5 : 2, false);
			pos = gap.position;
