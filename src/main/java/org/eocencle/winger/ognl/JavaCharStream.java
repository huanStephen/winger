...
public class JavaCharStream {
	...
	protected void ExpandBuff(boolean wrapAround) {
		...
		try {
			...
			throw new Error(arg5.getMessage());
		}
				...
				while (true) {
					...
					label80: {
						...
						try {
							...
							this.buffer[this.bufpos] = c = (char) (hexval(c) << 12 | hexval(this.ReadByte()) << 8
									| hexval(this.ReadByte()) << 4 | hexval(this.ReadByte()));
							this.column += 4;
	...
	public JavaCharStream(InputStream dstream, String encoding, int startline, int startcolumn, int buffersize)
		...
		this((Reader) (encoding == null ? new InputStreamReader(dstream) : new InputStreamReader(dstream, encoding)),
	...
	public void ReInit(InputStream dstream, String encoding, int startline, int startcolumn, int buffersize)
			throws UnsupportedEncodingException {
		this.ReInit(
				(Reader) (encoding == null ? new InputStreamReader(dstream) : new InputStreamReader(dstream, encoding)),
				startline, startcolumn, buffersize);
	}
	...
	public String GetImage() {
		return this.bufpos >= this.tokenBegin
				...
				: new String(this.buffer, this.tokenBegin, this.bufsize - this.tokenBegin)
	...
	public void adjustBeginLineColumn(int newLine, int newCol) {
		...
		for (columnDiff = 0; i < len; ++i) {
			arg9999 = this.bufline[j = start % this.bufsize];
			++start;
			int arg9;
			if (arg9999 != this.bufline[arg9 = start % this.bufsize]) {
				break;
			}

			this.bufline[j] = newLine;
			int arg10 = columnDiff + this.bufcolumn[arg9] - this.bufcolumn[j];
		...
		if (i < len) {
			...
			while (i++ < len) {
				arg9999 = this.bufline[j = start % this.bufsize];
				++start;
				if (arg9999 != this.bufline[start % this.bufsize]) {
					this.bufline[j] = newLine++;
				} else {
					this.bufline[j] = newLine;
				}
